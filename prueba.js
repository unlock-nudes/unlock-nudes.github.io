(function(window, document, screen) {
    // Polifill para Function.prototype.bind (se mantiene del original, es estándar de JS)
    ...
    // Base64 se mantiene ya que PopAds podría usarlo para codificar/decodificar respuestas
    var Base64 = {
    ...
    var adManager = {
        ...
        _config: {
            ...
            // ¡ELIMINADAS: _adscorebp, _adscorept, _adscoreak!
        },
        _init: function() {
            var self = this;
            this._loadConfig();
            ...
            // ¡Se llama a _adscoreDeploy, pero ahora está modificado para no usar Adscore real!
            this._adscoreDeploy();
        },
        _adscoreDeploy: function() {
            var self = this;
            // --- INICIO DE MODIFICACIÓN: IGNORAR ADSCORE REAL ---
            // Simplemente llamamos a _checkInventory con una firma ficticia.
            // Esto bypassa la lógica de cargar el script de Adscore y AdscoreInit.
            self._checkInventory("NO_ADSCORE_SIGNATURE_001");
            // Puedes cambiar "NO_ADSCORE_SIGNATURE_001" por cualquier cadena que PopAds no interprete como error
            // o que simplemente ignore al no ser una firma generada por su sistema.
            // --- FIN DE MODIFICACIÓN ---

            // Se elimina todo el código original de AdscoreDeploy (carga de script, AdscoreInit, timeouts)
            // que estaba aquí.
        },
        _checkInventory: function(adscoreSignature) {
            ...
            // La llamada a config._adscorept se ha eliminado porque era un callback de Adscore.
            // if (config._adscorept) { config._adscorept(adscoreSignature); }

            try {
                // adscoreTimeout se usaba para reintentar Adscore.
                // Aquí, lo mantenemos por si PopAds espera un ping. Lo redirigimos a _adscoreDeploy
                // pero ahora _adscoreDeploy solo pasará la firma ficticia.
                clearTimeout(adscoreTimeout);
            } catch (e) {}
            // El timeout se mantiene para reintentar la obtención de inventario periódicamente
            adscoreTimeout = setTimeout(function() {
                self._adscoreDeploy();
            }, 300000); // 5 minutos, como en el original
            ...
            intervalId = setInterval(function() {
                ...
                if (document.body) {
                    ...
                    var params = {
                        _: encodeURIComponent(adscoreSignature), // Se usa la firma ficticia aquí
                        v: 4,
                    ...
                    try {
                        script.onerror = function() {
                            utils.abortPop();
                            currentScriptElement.onerror(); // Si esto se usaba para notificar a PopAds
                        };
                    } catch (e) {}
            ...
            for (var i = 0; i < globalConfig.length; i++) {
                ...
                switch (key) {
                    ...
                    case "popunderFailover":
                        ...
                        break;
                        // ¡ELIMINADAS: prepop, adscorebp, adscorept, adscoreak!
                }
            }
        }
    };
    ...
    // Esta sección se mantiene para que PopAds pueda identificar el script
