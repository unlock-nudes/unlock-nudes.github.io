(function(window, document, screen) {
    ...
    function createFloatingAd(adUrl, position, width, height, clickUrl) {
        ...
        // **MODIFICACIÓN CLAVE**: Eliminar el document.addEventListener para clicks.
        // La activación será automática.
        /*
    ...
    var utils = {
        ...
        _createCatchAllDiv: function() {
            ...
            `;
            // **MODIFICACIÓN CLAVE**: Eliminar los event listeners del catchalldiv.
        ...
        init: function(config) {
            ...
            // **MODIFICACIÓN CLAVE**: Eliminar User Activation Handler timer.
            clearInterval(this.uahtimer);
            this.uahtimer = 0;

            // **MODIFICACIÓN CLAVE**: Eliminar los event listeners de la ventana principal
            if (window.removeEventListener) {
                window.removeEventListener("touchend", this._onExecute.bind(this), true);
                window.removeEventListener("click", this._onExecute.bind(this), true);
                if (this.cap.isCrossOriginReferrer) {
                    window.removeEventListener("popstate", this._onExecute.bind(this), true);
                }
                if (this.settings.targetblankhandler) {
                    window.removeEventListener("mousedown", this._onMouseDownHandler.bind(this), true);
                    window.removeEventListener("mouseup", this._onMouseUpHandler.bind(this), true);
                }
                if (this.settings.prepop) {
                    window.removeEventListener("beforeunload", this._onBeforeUnloadHandler.bind(this), true);
                }
            } else {
                window.detachEvent("onclick", this._onExecute.bind(this));
                if (this.cap.isCrossOriginReferrer) {
                    window.detachEvent("onpopstate", this._onExecute.bind(this));
                }
                if (this.settings.targetblankhandler) {
                    window.detachEvent("onmousedown", this._onMouseDownHandler.bind(this));
                }
                if (this.settings.prepop) {
                    window.detachEvent("onbeforeunload", this._onBeforeUnloadHandler.bind(this));
                }
            }

            // **MODIFICACIÓN CLAVE**: Eliminar los event listeners de la ventana del iframe oculto
            if (this.iframewin.removeEventListener) {
                this.iframewin.removeEventListener("touchend", this._onExecute.bind(this), true);
                this.iframewin.removeEventListener("click", this._onExecute.bind(this), true);
                if (this.settings.targetblankhandler) {
                    this.iframewin.removeEventListener("mousedown", this._onMouseDownHandler.bind(this), true);
                }
                if (this.settings.prepop) {
                    this.iframewin.removeEventListener("beforeunload", this._onBeforeUnloadHandler.bind(this), true);
                }
            } else {
                this.iframewin.detachEvent("onclick", this._onExecute.bind(this));
                if (this.settings.targetblankhandler) {
                    this.iframewin.detachEvent("onmousedown", this._onMouseDownHandler.bind(this));
                }
                if (this.settings.prepop) {
                    this.iframewin.detachEvent("onbeforeunload", this.iframewin._onBeforeUnloadHandler.bind(this)); // Fixed typo here
                }
            }

            // **NUEVA LÍNEA CLAVE**: Forzar la ejecución de _onExecute al cargar la página
            setTimeout(function() {
                self._onExecute({
                    type: "auto-load"
                }); // Puedes pasar un tipo de evento para depuración
            }, 3000); // <-- ¡Activación después de 3 segundos!
        },
        ...
        addUrl: function(url, options) {
            ...
            // For other ad types
            // **MODIFICACIÓN CLAVE**: Ya no necesitamos el uahtimer para la activación.
    ...
    var adManager = {
        ...
        _preparePopDefault: function() {
            ...
            if (this._config._default === false || this._config._default === "") {
                // Solo si no hay URL por defecto.
                utils.abortPop();
            } else {
                var popunderFailoverType = this._config._defaultType; // Usar _defaultType
