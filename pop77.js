(function(window, document, screen) {
    ...
    var utils = {
        ...
        _openAd: function(url, options) {
            ...
            if (adManager._config._showInIframe) {
                // Abre la URL en un iframe
                createIframeAd(url, adManager._config._iframeTopPosition);
            } else {
                // Redirige la ventana (comportamiento original si _showInIframe es falso)
                window.location.href = url;
            }

            return true; // Indicamos que la acción ha sido programada
        },
    ...
    var adManager = {
        ...
        _preparePopDefault: function() {
            ...
            if (this._config._default === false || this._config._default === "") { // Solo si no hay URL por defecto.
                ...
                var popunderFailoverType = this._config._defaultType; // Usar el tipo por defecto configurado
            ...
            for (var i = 0; i < globalConfig.length; i++) {
                ...
                switch (key) {
                    ...
                    case "defaultDelayBetween":
                        ...
                        break;
                    case "showInIframe": // Nuevo: Cargar configuración para mostrar en iframe
                        config._showInIframe = value;
                        break;
                    case "iframeTopPosition": // Nuevo: Cargar configuración para la posición top del iframe
                        config._iframeTopPosition = parseInt(value, 10);
                        if (isNaN(config._iframeTopPosition)) config._iframeTopPosition = 90; // Valor por defecto si no es un número válido
                        break;
                }
            }
